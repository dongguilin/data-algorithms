 ...
 * <p>
 * 二次排序
 * 在Hadoop中，如果需要持久存储定制数据类型（如DateTemperaturePair）,则；实现Writable接口；
 * 如果要比较定制数据类型必须实现接口WritableComparable
 *
...
public class DateTemperaturePair
    ...
    private final Text yearMonth = new Text();//自然键
    private final Text day = new Text();
    private final IntWritable temperature = new IntWritable();//次键
