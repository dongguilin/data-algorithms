...
object MovieRecommendations {
  def main(args: Array[String]): Unit = {
    ...
    //TSV(tab-separated value,tab分隔值)文件
    //用户  电影  评分
    //<user>  <movie>  <rating>
    val input = args(0)
    val output = args(1)
    ...
    //(user, movie, rating, numOfRaters)
    val userMovieRatingNumberOfRater = userMovieRating.map(umr => (umr._2, (umr._1, umr._3)))
      .join(numberOfRatersPerMovie)
    ...
    // ((movie1, movie2),  (rating1, numOfRaterS1, rating2, numOfRaterS2, ratingProduct, rating1Squared, rating2Squared))
    val moviePairs = groupedByUser.flatMap(tuple => {
      val sorted = tuple._2.toList.sortBy(f => f._1) //sorted by movies
      println(sorted)
      val tuple7 = for {
        movie1 <- sorted
        movie2 <- sorted if (movie1._1 < movie2._1); // avoid duplicate
